...
- **Play-test UX (July 2026)**: live made-hand label under the hero's nameplate (`src/engine/hand-label.ts`, pure + client-safe, hold'em card counts 2/5/6/7 only — preflop is just the pocket-pair check; rendered in `Seat.tsx`, emerald = live vs amber = showdown result). Last-used name prefills create/join via `useRememberedName` (localStorage `hg:playerName`, `useSyncExternalStore` for clean hydration).
