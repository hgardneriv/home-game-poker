...
4. **Try Simulator first, then the phone** — on a Mac: pull this branch, `npx cap sync ios`, `npx cap run ios` (or Xcode destination → iPhone simulator → Play). Icon + **Texas Hold’em** label are native. Privacy **Close** is a website change — the sim still loads `https://holdem.pokerparty.app`, so Close will not appear until this branch is production-deployed (`vercel deploy --prod` from `iphone-app` after merge). Then the same Xcode Play target can be your iPhone.
